...
/** OpenGraph locale tag per app locale (home/default metadata). */
const OG_LOCALE: Record<string, string> = { en: "en_US", es: "es_ES", pt: "pt_BR" };

// Per-locale so `metadataBase`, `alternates` and `openGraph.url` reflect the
// current locale's subdomain. Depends only on `params.lang` (a static param), so
// it stays prerenderable under cacheComponents — no `unstable_instant` needed.
export async function generateMetadata({
  ...
  return {
    metadataBase: new URL(localeUrl(locale)),
    title: {
...
export default async function RootLayout({
  ...
  const { lang } = await params;
  // Unknown/unsupported locale segments fall back to English rather than 404ing,
  // so a stray `/fr/...` URL still renders usable (English) content.
  const locale = isLocale(lang) ? lang : defaultLocale;
